...
public class ChatSignalRService : IAsyncDisposable, ISignalRService
{
    ...
    {
        _hubUrl = "http://localhost:5096/chathub";
    ...
    public async Task StartAsync()
    {
        ...
        // Écoute des messages
        _hubConnection.On<int, int, string, DateTime>("ReceiveMessage", (conversationId, senderId, message, date) =>
        {
            OnMessageReceived?.Invoke(conversationId, senderId, message, date);
        });

        _hubConnection.On<int, int, string>("UserIsTyping", (conversationId, userId, userName) =>
        {
            OnUserTyping?.Invoke(conversationId, userId, userName);
        });

        _hubConnection.On<int, int>("MessagesRead", (conversationId, userId) =>
        {
...
//     public ChatSignalRService()
//     {
//         _hubUrl = "https://localhost:5001/chathub";
//         Console.WriteLine("[SignalR] Service initialized with hub URL: " + _hubUrl);
//     }
//
//     event Action<int, int, string, DateTime>? ISignalRService.OnMessageReceived
//     {
